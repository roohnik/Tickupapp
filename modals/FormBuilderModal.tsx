...
import {
  ...
  User,
  CalculationSettings,
  NumberCalculationCondition,
  NumberCalculationRule,
  ValueScore,
  OptionScore,
  FormVariable,
} from "../types";
...
const calculableFieldTypes: FormFieldType[] = [
  "SELECT",
  "RADIO",
  "RATING",
  "CONFIRMATION",
...
const CalculationsPanel: React.FC<CalculationsPanelProps> = ({
  fields,
  setFields,
  enableCalculations,
  setEnableCalculations,
  maxScore,
  setMaxScore,
}) => {
  const calculableFields = fields.filter((f) =>
    calculableFieldTypes.includes(f.type)
  );

  const updateFieldCalculation = (
    fieldId: string,
    config: CalculationSettings | undefined
  ) => {
    ...
    const calculateMaxScore = (): number => {
      ...
      calculableFields.forEach((field) => {
        ...
        if (config.optionScores) {
          fieldMax = Math.max(
            0,
            ...config.optionScores.map((s) => s.score || 0)
          );
        } else if (config.valueScores) {
          fieldMax = Math.max(
            0,
            ...config.valueScores.map((s) => s.score || 0)
          );
          ...
          fieldMax = Math.max(
            0,
            ...Object.values(config.ratingScores).map((s) => Number(s) || 0)
          );
        } else if (config.numberRules) {
          fieldMax = Math.max(
            config.numberRules.defaultScore || 0,
            ...config.numberRules.rules.map((r) => r.score || 0)
          );
        }
        totalMax += fieldMax;
      });
  ...
  const renderValueScoreInput = (
    field: FormField,
    value: boolean | string,
    label: string
  ) => {
    const score =
      field.calculationConfig?.valueScores?.find((s) => s.value === value)
        ?.score ?? "";
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      const newScore =
        e.target.value === "" ? undefined : parseInt(e.target.value, 10);
      const currentScores: ValueScore[] =
        field.calculationConfig?.valueScores || [];
      const existing = currentScores.find((s) => s.value === value);
      let updatedScores;
      if (existing) {
        updatedScores = currentScores.map((s) =>
          s.value === value ? { ...s, score: newScore ?? 0 } : s
        );
      } else {
        updatedScores = [...currentScores, { value, score: newScore ?? 0 }];
      }
      updateFieldCalculation(field.id, {
  ...
  const handleNumberRuleChange = (
    fieldId: string,
    ruleId: string,
    updates: Partial<NumberCalculationRule>
  ) => {
    const config = fields.find((f) => f.id === fieldId)?.calculationConfig;
    if (!config?.numberRules) return;
    const newRules = config.numberRules.rules.map((r) =>
      r.id === ruleId ? { ...r, ...updates } : r
    );
    updateFieldCalculation(fieldId, {
      ...config,
      numberRules: { ...config.numberRules, rules: newRules },
    });
  };

  const addNumberRule = (fieldId: string) => {
    const config = fields.find((f) => f.id === fieldId)?.calculationConfig;
    const newRule: NumberCalculationRule = {
      id: `rule-${Date.now()}`,
      condition: "EQUALS",
      value1: 0,
      score: 0,
    ...
    const newNumberRules = {
      defaultScore: config?.numberRules?.defaultScore || 0,
      rules: newRules,
    };
      ...
      <div className="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div>
          ...
          <p className="text-sm text-blue-600">
            برای فیلدهای فرم امتیاز تعریف کنید تا در نهایت امتیاز کل محاسبه شود.
          </p>
        </div>
        <input
          type="checkbox"
          ...
          <div className="space-y-4">
            {calculableFields.length > 0 ? (
                ...
                <div key={field.id} className="p-4 border rounded-lg bg-white">
                  ...
                  {["SELECT", "RADIO"].includes(field.type) && (
                    <div className="space-y-2">
                      {field.options?.map((option) => {
                        const score =
                          field.calculationConfig?.optionScores?.find(
                          ...
                          >
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                const currentScores: OptionScore[] =
                                  field.calculationConfig?.optionScores || [];
                  ...
                  {field.type === "RATING" && (
                    <div className="space-y-2">
                      {[1, 2, 3, 4, 5].map((star) => {
                        const score =
                          field.calculationConfig?.ratingScores?.[star] ?? "";
                        return (
                          <div
                            key={star}
                            className="flex items-center justify-between"
                              ...
                              onChange={(e) => {
                                const newScore =
                                  e.target.value === ""
                                    ...
                                    : parseInt(e.target.value, 10);
                                const currentScores =
                                  field.calculationConfig?.ratingScores || {};
                                const updatedScores = {
                                  ...currentScores,
                                  [star]: newScore ?? 0,
                                };
                  ...
                  {field.type === "NUMBER" &&
                    (() => {
                      const rules =
                        field.calculationConfig?.numberRules?.rules || [];
                      const defaultScore =
                        field.calculationConfig?.numberRules?.defaultScore ?? 0;
                      const conditions: {
                        value: NumberCalculationCondition;
                        label: string;
                      }[] = [
                      ...
                      return (
                        <div className="space-y-3">
                          <p className="text-xs text-gray-500">
                            اگر عدد ورودی با یکی از شرایط زیر مطابقت داشت،
                            امتیاز مربوطه را دریافت می‌کند.
                          </p>
                          {rules.map((rule) => (
                            <div
                              key={rule.id}
                              className="flex items-center flex-wrap gap-2 p-2 bg-gray-100/70 rounded-md"
                            >
                              <span>اگر</span>
                              <select
                                value={rule.condition}
                                onChange={(e) =>
                                  handleNumberRuleChange(field.id, rule.id, {
                                    condition: e.target
                                      .value as NumberCalculationCondition,
                                  })
                                }
                                className="p-1 border-gray-300 rounded text-xs"
                              >
                                {conditions.map((c) => (
                              ...
                              <input
                                ...
                                className="w-16 p-1 border-gray-300 rounded text-xs"
                              />
                              ...
                              <button
                                onClick={() =>
                                  removeNumberRule(field.id, rule.id)
                                }
                                className="p-1 text-gray-400 hover:text-red-500 mr-auto"
                              >
                          ...
                          <button
                            onClick={() => addNumberRule(field.id)}
                            className="text-sm text-blue-600 font-semibold flex items-center"
                          >
                            <PlusIcon className="w-4 h-4 ml-1" /> افزودن شرط
                          </button>
                          <div className="border-t pt-3 mt-3 flex items-center justify-between">
                            <label className="text-sm">
                              امتیاز پیش‌فرض (اگر هیچ شرطی برقرار نبود)
                            </label>
                            <input
                              type="number"
                              value={defaultScore}
                              onChange={(e) => {
                                const config = field.calculationConfig;
                                const newNumberRules = {
                                  rules: config?.numberRules?.rules || [],
              ...
              <p className="text-center text-gray-500 py-8">
                هیچ فیلد قابل امتیازبندی در فرم شما وجود ندارد.
              </p>
            )}
          </div>
        </div>
...
const VariablesPanel: React.FC<VariablesPanelProps> = ({
  ...
  const handleSave = () => {
    ...
    if (isUpdating) {
      setVariables((vars) =>
        vars.map((v) =>
          v.id === editState.id ? (editState as FormVariable) : v
        )
      );
    } else {
  ...
  const renderEditPanel = () => (
    <div className="p-4 space-y-4">
      <h3 className="font-semibold">
        {editState.id?.startsWith("var-") ? "متغییر جدید" : "ویرایش متغییر"}
      </h3>
      <div>
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            setEditState((p) => ({ ...p, purpose: e.target.value }))
          }
          className="input-style mt-1"
        />
      </div>
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            setEditState((p) => ({ ...p, label: e.target.value }))
          }
          className="input-style mt-1"
        />
      </div>
      ...
      <div>
        ...
        <div className="relative mt-1" ref={fieldDropdownRef}>
          <button
            type="button"
            onClick={() => setIsFieldDropdownOpen((p) => !p)}
            className="input-style w-full text-right flex justify-between items-center"
          >
            <span className="truncate">
              {editState.fieldIds?.length || 0} سوال انتخاب شده
            </span>
            ...
            <div className="absolute top-full mt-1 w-full bg-white border rounded-md shadow-lg z-10 max-h-48 overflow-y-auto">
              {calculableFields.map((field) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={editState.fieldIds?.includes(field.id)}
                    onChange={() => toggleFieldInVariable(field.id)}
      ...
      <div className="flex items-center space-x-2 space-x-reverse pt-2">
        <button
          onClick={handleSave}
          className="px-4 py-1.5 bg-brand-primary text-white text-sm rounded-md"
        >
          ذخیره
        </button>
        <button
          ...
          className="px-4 py-1.5 bg-gray-200 text-sm rounded-md"
        >
          لغو
        </button>
      </div>
        ...
        {isEditing ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-sm text-gray-500 max-w-xs">
              متغییرها به شما کمک می‌کنند تا سوالات مرتبط را دسته‌بندی کرده و
              امتیاز آن‌ها را به صورت جداگانه در داشبورد مشاهده کنید.
            </p>
            <button
              onClick={startNewVariable}
              className="mt-4 px-4 py-2 bg-brand-primary text-white font-semibold rounded-lg text-sm flex items-center"
            >
              <PlusIcon className="w-4 h-4 ml-1" /> ایجاد متغییر جدید
            </button>
          </div>
        )}
      </div>
  ...
  const newField: FormField = {
    ...
    label:
      type === "SECTION"
        ...
        : type === "DYNAMIC_TABLE"
        ? "جدول جدید"
        : `فیلد ${type.toLowerCase()}`,
    isRequired: false,
    ...((type === "SELECT" || type === "RADIO" || type === "CHECKBOX") && {
      options: [{ id: "opt1", label: "گزینه ۱" }],
    }),
    ...(type === "MATRIX_SINGLE" && {
      matrixRows: [{ id: "row1", label: "ردیف ۱" }],
      matrixColumns: [{ id: "col1", label: "ستون ۱" }],
    }),
    ...(type === "DYNAMIC_TABLE" && {
      subFields: [
        {
...
//   currentUser,
// }) => {
const FormBuilderModal: React.FC = observer(() => {
  const { uiStore, formStore, settingsStore, userStore } = useStore();
  const isOpen = uiStore.isOpen("formBuilder");
  const formToEdit = formStore.formToEdit;
  const categories = formStore.categories;
  const styleSettings = settingsStore.componentStyles;
  ...
  const [displayMode, setDisplayMode] = useState<"SINGLE_PAGE" | "MULTI_STEP">(
    "SINGLE_PAGE"
  );

  // State for "Create from Text" tab
  const [textInput, setTextInput] = useState("");
  const [selectedFieldTypeForText, setSelectedFieldTypeForText] =
    useState<FormFieldType>("TEXT");
  const [textPattern, setTextPattern] = useState<
    "single" | "multi" | "slash" | "qa"
  >("single");
  ...
  const duplicateField = (id: string) => {
    ...
    if (newField.options) {
      newField.options = newField.options.map((opt: FormFieldOption) => ({
        ...opt,
        id: `opt-${Date.now()}-${Math.random()}`,
      }));
    }
    if (newField.matrixRows) {
      newField.matrixRows = newField.matrixRows.map((opt: FormFieldOption) => ({
        ...opt,
        id: `row-${Date.now()}-${Math.random()}`,
      }));
    }
    if (newField.matrixColumns) {
      newField.matrixColumns = newField.matrixColumns.map(
        (opt: FormFieldOption) => ({
          ...opt,
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (
      type &&
      FIELD_PALETTE_ITEMS.some((cat) => cat.fields.some((f) => f.type === type))
    ) {
      const newField = createNewFieldObject(type);

      // Find which field we dropped on
      const droppedOnElement = (e.target as HTMLElement).closest(
        "[data-field-id]"
      );
  ...
  const handleMatrixOptionUpdate = (
    fieldId: string,
    type: "matrixRows" | "matrixColumns",
    optionId: string,
    newLabel: string
  ) => {
    setFields((prev) =>
      prev.map((f) => {
        if (f.id === fieldId && f[type]) {
          return {
            ...f,
            [type]: (f[type] as FormFieldOption[]).map((o) =>
        ...
        if (f.id === fieldId) {
          ...
          return {
            ...
            [type]: [
              ...
              {
                id: `${type}-${Date.now()}`,
                label: `${prefix} ${currentOptions.length + 1}`,
              },
            ],
          };
        }
    ...
    setFields((prev) =>
      prev.map((f) => {
        if (f.id === fieldId && f[type]) {
          return {
            ...f,
            [type]: (f[type] as FormFieldOption[]).filter(
              (o) => o.id !== optionId
            ),
          };
        }
        ...
        if (f.id === fieldId && f.options) {
          return {
            ...
            options: [
              ...
              {
                id: `opt-${Date.now()}`,
                label: `گزینه ${f.options.length + 1}`,
              },
            ],
          };
        }
  ...
  const handleSubmit = (formData) => {
    ...
    uiStore.closeModal("formBuilder");
    };
//   const handleSubmit = () => {
...
//   };
    ...
    if (textPattern === "single") {
      ...
      for (let i = 0; i < lines.length; i += 2) {
        ...
        if (label) {
          ...
          newFields.push(
            createField(label, selectedFieldTypeForText, placeholder)
          );
        }
      }
      ...
      if (slashIndex === -1) {
        ...
        titles.forEach((title, index) => {
          ...
          newFields.push(
            createField(title, selectedFieldTypeForText, placeholder)
          );
        });
      }
      ...
      if (slashIndices.length < 2) {
        alert(
          "الگوی سوال و پاسخ نیاز به حداقل دو جداکننده / دارد: یکی پس از عنوان‌ها و دیگری پس از راهنماها."
        );
        return;
      }

      const titles = lines
        .slice(0, slashIndices[0])
        .map((l) => l.trim())
        .filter((l) => l);
      const placeholders = lines
        .slice(slashIndices[0] + 1, slashIndices[1])
        .map((l) => l.trim())
        .filter((l) => l);
      const types = lines
        .slice(slashIndices[1] + 1)
        .map((l) => l.trim())
        .filter((l) => l);
      ...
      titles.forEach((title, index) => {
        ...
        const fieldType =
          typeCode && typeMap[typeCode] ? typeMap[typeCode] : "TEXT";
  ...
  const handleGenerateFromAI = async () => {
    ...
    try {
      ...
      const newFields: FormField[] = generated.map((genField, index) => ({
        ...
        icon: genField.icon,
        options: genField.options?.map((opt, i) => ({
          id: `opt-${Date.now()}-${index}-${i}`,
          label: opt,
        })),
        matrixRows: genField.matrixRows?.map((opt, i) => ({
          id: `row-${Date.now()}-${index}-${i}`,
          label: opt,
        })),
        matrixColumns: genField.matrixColumns?.map((opt, i) => ({
          id: `col-${Date.now()}-${index}-${i}`,
          label: opt,
        })),
      }));
  ...
  const hasCalculableFields = fields.some((f) =>
    calculableFieldTypes.includes(f.type)
  );

  const handleSubFieldUpdate = (
    fieldId: string,
    subFieldId: string,
    ...
    <div className="max-w-3xl mx-auto p-4 sm:p-8">
      <div className="mb-6 border rounded-lg">
        <button
          ...
          className="w-full flex justify-between items-center p-3 bg-gray-100/60 rounded-t-lg"
        >
        ...
        {isSpecVisible && (
          <div className="p-4 space-y-4 animate-fade-in">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-medium text-brand-subtext">
                  کد فرم (برای لینک یکتا)
                </label>
                <input
                  type="text"
                  value={formCode}
                  onChange={(e) => setFormCode(e.target.value)}
                  className="w-full text-sm border-gray-200 rounded-md p-2 mt-1"
                />
                {generatedLink && (
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setDocumentRequestNumber(e.target.value)}
                className="w-full text-sm border-gray-200 rounded-md p-2 mt-1"
              />
            </div>
            {hasSections && (
      ...
      <div className="mb-8">
        <input
          ...
          className="text-2xl md:text-3xl font-bold w-full border-none focus:ring-0 p-1 -m-1"
        />
        <textarea
          value={description}
          onChange={(e) => setDescription(e.target.value)}
          placeholder="توضیحات فرم (اختیاری)"
          className="text-sm text-brand-subtext w-full border-none focus:ring-0 resize-none p-1 -m-1 mt-2"
          rows={1}
        ></textarea>
      </div>
      ...
      <div
        ...
        className="space-y-4 min-h-[200px] border-2 border-dashed rounded-lg p-4"
      >
        {fields.map((field, index) => {
          if (field.type === "SECTION") {
            const SectionIcon =
              ICONS[field.icon || "ViewColumnsIcon"] || ViewColumnsIcon;
            return (
              <div
                key={field.id}
                ...
                onDragStart={(e) => {
                  ...
                  e.dataTransfer.setData("internal-move", String(index));
                }}
                onDragEnter={() => (dragOverItem.current = index)}
                onDragEnd={handleSort}
                onDragOver={(e) => e.preventDefault()}
                className={`py-2 my-4 border-t-2 border-b-2 rounded-lg cursor-pointer transition-colors ${
                  selectedFieldId === field.id && !isMobile
            ...
            return (
              <div
                ...
                className={`p-4 rounded-lg cursor-pointer border-2 transition-colors ${
                  selectedFieldId === field.id && !isMobile
                    ? "border-brand-primary bg-blue-50/50"
                    : "border-transparent bg-gray-100/70 hover:border-gray-300"
                }`}
              >
                <div className="flex justify-between items-start">
        ...
        {fields.length === 0 && (
          <p className="text-center text-brand-subtext">
            برای شروع، یک فیلد را از پنل کناری به اینجا بکشید یا از تب "ایجاد از
            روی متن" استفاده کنید.
          </p>
        )}
      </div>
    </div>
      ...
      <div className="p-4 space-y-4">
        ...
        {!isMobile && (
          <button
            ...
            className="w-full mt-4 py-1.5 text-sm bg-gray-200 rounded-md hover:bg-gray-300"
          >
            بازگشت به فیلدها
          </button>
        )}
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center">
          <input
            ...
            className="ml-2 rounded text-brand-primary"
          />
        ...
        {(selectedField.type === "SELECT" ||
          ...
          <div className="border-t pt-4">
            ...
            <div className="space-y-2">
              {selectedField.options?.map((opt) => (
                <div key={opt.id} className="flex items-center">
                  ...
                  <button
                    type="button"
                    onClick={() => removeOption(selectedField.id, opt.id)}
            ...
            <button
              ...
              className="mt-2 text-sm text-brand-primary flex items-center"
            >
              <PlusIcon className="w-4 h-4 ml-1" /> افزودن گزینه
            </button>
          </div>
        )}
        ...
        {selectedField.type === "MATRIX_SINGLE" && (
          <div className="border-t pt-4 space-y-4">
            <div>
              ...
              <button
                type="button"
                onClick={() => addMatrixOption(selectedField.id, "matrixRows")}
                className="mt-2 text-sm text-brand-primary flex items-center"
              >
                <PlusIcon className="w-4 h-4 ml-1" /> افزودن ردیف
              </button>
            </div>
            <div>
              ...
              <button
                type="button"
                onClick={() =>
                  addMatrixOption(selectedField.id, "matrixColumns")
                }
                className="mt-2 text-sm text-brand-primary flex items-center"
              >
                <PlusIcon className="w-4 h-4 ml-1" /> افزودن ستون
              </button>
            </div>
          </div>
        )}
        {selectedField.type === "DYNAMIC_TABLE" && (
          <div className="border-t pt-4">
              ...
              {selectedField.subFields?.map((sf) => (
                ...
                >
                  <input
                    ...
                    value={sf.label}
                    onChange={(e) =>
                      handleSubFieldUpdate(selectedField.id, sf.id, {
                  ...
                  <button
                    type="button"
                    onClick={() => removeSubField(selectedField.id, sf.id)}
                    className="p-1 text-gray-400 hover:text-red-500"
                  >
                    <TrashIcon className="w-4 h-4" />
                  </button>
            ...
            <button
              type="button"
              onClick={() => addSubField(selectedField.id)}
              className="mt-2 text-sm text-brand-primary flex items-center"
            >
              <PlusIcon className="w-4 h-4 ml-1" /> افزودن ستون
            </button>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    handleFieldUpdate(selectedField.id, {
                      signerUserIds: newSigners.filter(Boolean),
                    });
                  }}
                  className="mt-1 w-full text-sm border-gray-300 rounded-md"
                >
    ...
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-0 md:p-4">
      <div className="bg-brand-secondary rounded-none md:rounded-lg w-full h-full max-w-6xl max-h-full md:max-h-[90vh] flex flex-col">
        ...
        <div className="flex-shrink-0 flex justify-between items-center p-3 border-b bg-white rounded-t-lg">
            ...
            <div className="flex items-center space-x-1 space-x-reverse p-1 bg-gray-200/60 rounded-lg">
              ...
              <button
                ...
                className={`flex items-center px-3 py-1 text-sm rounded-md ${
                  tab === "variables" ? "bg-white shadow-sm" : ""
                }`}
              >
              ...
              {hasCalculableFields && (
                <button
                  ...
                  className={`flex items-center px-3 py-1 text-sm rounded-md ${
                    tab === "calculations" ? "bg-white shadow-sm" : ""
                  }`}
                >
              ...
              <button
                type="button"
                onClick={() => setTab("fromText")}
                className={`px-3 py-1 text-sm rounded-md ${
                  tab === "fromText" ? "bg-white shadow-sm" : ""
                }`}
              >
                ایجاد از روی متن
              </button>
              <button
                type="button"
                onClick={() => setTab("aiCreate")}
                className={`px-3 py-1 text-sm rounded-md ${
                  tab === "aiCreate" ? "bg-white shadow-sm" : ""
                }`}
              >
                ایجاد هوشمند
              </button>
              <button
                ...
                className={`px-3 py-1 text-sm rounded-md ${
                  tab === "preview" ? "bg-white shadow-sm" : ""
                }`}
              >
                پیش‌نمایش
              </button>
          ...
          <div className="flex items-center space-x-4 space-x-reverse">
            <button
              ...
              className="px-4 py-1.5 bg-brand-primary text-white text-sm font-semibold rounded-lg"
            >
        ...
        <div className="flex-grow flex flex-col md:flex-row overflow-hidden relative">
          {/* Canvas / Preview */}
          <div
            className={`flex-grow bg-white overflow-y-auto ${
              isMobile && mobileView !== "canvas" ? "hidden" : "block"
            }`}
          >
            ...
            {tab === "calculations" && (
              <CalculationsPanel
                ...
                setEnableCalculations={setEnableCalculations}
                maxScore={maxScore}
                  ...
                  {fields.map((field) => (
                    <FormFieldRenderer
                      ...
                      displayStyle={
                        hasSections
                          ? "MINIMAL_CARD"
                          : styleSettings.formDisplayStyle || "DEFAULT"
                      }
                      styleSettings={styleSettings}
              ...
              <div className="p-4 sm:p-8 space-y-6 max-w-3xl mx-auto animate-fade-in">
                <div className="text-center">
                  <h2 className="text-2xl font-bold text-brand-text">
                    ایجاد فرم با هوش مصنوعی
                  </h2>
                  <p className="text-brand-subtext mt-2">
                    یک فایل بارگذاری کنید یا فرم مورد نظر خود را توصیف کنید تا
                    هوش مصنوعی آن را برای شما بسازد.
                  </p>
                </div>
                  ...
                  <div className="p-4 border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-center">
                    ...
                    {uploadedFile && (
                      <p className="text-sm mt-2 text-green-600">
                        فایل انتخاب شد: {uploadedFile.name}
                      </p>
                    )}
                  </div>
                ...
                <div className="text-center">
                  ...
                  >
                    {isGenerating ? (
                      ...
                      <>
                        <SparklesIcon className="w-5 h-5 ml-2" /> ساخت فرم
                      </>
                    )}
                  </button>
                </div>
            ...
            {tab === "fromText" && (
              <div className="p-4 sm:p-8 space-y-4 max-w-3xl mx-auto animate-fade-in">
                ...
                <p className="text-sm text-brand-subtext">
                  سوالات خود را در کادر زیر وارد کنید. بر اساس تنظیمات، هر سطر
                  یا هر چند سطر به یک فیلد تبدیل می‌شود.
                </p>
                <textarea
                  value={textInput}
                ...
                <div className="pt-2 space-y-3">
                  ...
                  <div className="p-2 rounded-md border has-[:checked]:border-brand-primary has-[:checked]:bg-blue-50/50">
                    <label className="flex items-center cursor-pointer">
                      <input
                        type="radio"
                        name="textPattern"
                        value="single"
                        checked={textPattern === "single"}
                        onChange={() => setTextPattern("single")}
                        className="ml-2 h-4 w-4 text-brand-primary focus:ring-brand-primary"
                      />
                      <span className="font-semibold">هر سطر یک فیلد</span>
                    </label>
                  </div>
                  <div className="p-2 rounded-md border has-[:checked]:border-brand-primary has-[:checked]:bg-blue-50/50">
                    <label className="flex items-center cursor-pointer">
                      <input
                        ...
                        name="textPattern"
                        value="multi"
                        checked={textPattern === "multi"}
                        onChange={() => setTextPattern("multi")}
                        className="ml-2 h-4 w-4 text-brand-primary focus:ring-brand-primary"
                      />
                      <span className="font-semibold">الگوی دو خطی</span>
                    </label>
                    {textPattern === "multi" && (
                      <p className="text-xs text-gray-500 mt-1 pl-6">
                        هر ۲ سطر یک فیلد را تشکیل می‌دهد: سطر اول{" "}
                        <strong>عنوان فیلد</strong> و سطر دوم{" "}
                        <strong>متن راهنما (Placeholder)</strong> خواهد بود.
                      </p>
                    )}
                  </div>
                  <div className="p-2 rounded-md border has-[:checked]:border-brand-primary has-[:checked]:bg-blue-50/50">
                    <label className="flex items-center cursor-pointer">
                      <input
                        ...
                        name="textPattern"
                        value="slash"
                        checked={textPattern === "slash"}
                        onChange={() => setTextPattern("slash")}
                        className="ml-2 h-4 w-4 text-brand-primary focus:ring-brand-primary"
                      />
                      <span className="font-semibold">الگوی اسلش</span>
                    </label>
                    {textPattern === "slash" && (
                      <p className="text-xs text-gray-500 mt-1 pl-6">
                        هر سطر قبل از اسلش یک <strong>عنوان فیلد</strong> است.
                        سطرهای بعد از اسلش به ترتیب{" "}
                        <strong>متن‌های راهنما</strong> برای فیلدهای بالا هستند.
                      </p>
                    )}
                  </div>
                  <div className="p-2 rounded-md border has-[:checked]:border-brand-primary has-[:checked]:bg-blue-50/50">
                    <label className="flex items-center cursor-pointer">
                      <input
                        ...
                        name="textPattern"
                        value="qa"
                        checked={textPattern === "qa"}
                        onChange={() => setTextPattern("qa")}
                        className="ml-2 h-4 w-4 text-brand-primary focus:ring-brand-primary"
                      />
                      <span className="font-semibold">الگوی سوال و پاسخ</span>
                    </label>
                    {textPattern === "qa" && (
                      <div className="text-xs text-gray-500 mt-1 pl-6 space-y-2">
                        <p>
                          متن خود را به سه بخش تقسیم کنید که با یک / در یک خط
                          جدا از هم جدا شده‌اند. بخش اول{" "}
                          <strong>عنوان‌ها</strong>، بخش دوم{" "}
                          <strong>راهنماها</strong>، و بخش سوم{" "}
                          <strong>کد نوع فیلد</strong> است.
                        </p>
                        <details>
                ...
                <div
                  className={`flex items-center space-x-4 space-x-reverse pt-2 ${
                    textPattern === "qa" ? "opacity-50" : ""
                  }`}
                >
                  <label
                    className={`text-sm font-medium ${
                      textPattern === "qa" ? "text-gray-400" : ""
                    }`}
                  >
                    نوع فیلدها:
                  </label>
                  ...
                  >
                    {FIELD_PALETTE_ITEMS.flatMap((cat) => cat.fields).map(
                      (f) => (
          ...
          <div
            className={`flex-shrink-0 w-full md:w-80 border-r bg-gray-100/70 overflow-y-auto ${
              isMobile && mobileView === "canvas" ? "hidden" : "block"
            }`}
          >
            {isMobile && (
              <div className="p-2 border-b flex items-center justify-between bg-white">
                <button type="button" onClick={() => setMobileView("canvas")}>
                ...
                <div className="flex items-center space-x-1 space-x-reverse bg-gray-200 p-1 rounded-lg">
                  <button
                    ...
                    className={`px-3 py-1 text-sm rounded-md ${
                      mobileView === "palette" ? "bg-white shadow" : ""
                    }`}
                  >
                    فیلدها
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 text-sm rounded-md ${
                      mobileView === "settings" ? "bg-white shadow" : ""
            ...
            <div
              className={`${
                isMobile && mobileView !== "palette" ? "hidden" : "block"
              }`}
            >
            ...
            <div
              className={`${
                isMobile && mobileView === "settings" ? "block" : "hidden"
              }`}
            >
              {settingsContent}
            </div>
