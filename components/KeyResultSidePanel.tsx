...
import { KeyResult, KRCategory, FeedbackTag, Objective, Project, Task, User, KRType, Comment } from '../types';
import { CloseIcon, CheckCircleIcon, XCircleIcon, TrophyIcon, TagIcon, ICONS, PaperAirplaneIcon } from './Icons';
...
interface KeyResultSidePanelProps {
    isOpen: boolean;
    onClose: () => void;
    kr: KeyResult;
    objectiveId: string;
    onCheckin: (objectiveId: string, krId: string, value: number, rating: number, report: { tasksDone: string; tasksNext: string; challenges: string; }, challengeDifficulty: number, challengeTagIds: string[]) => void;
...
const KeyResultSidePanel: React.FC<KeyResultSidePanelProps> = ({ isOpen, onClose, kr, objectiveId, onCheckin, onAddComment, challengeTags, objectives, projects, tasks, users, currentUser, onSelectTask }) => {
    ...
    }, [objectiveId, objectives, projects]);
    ...
    const handleCheckin = (e: React.FormEvent) => {
        ...
        onCheckin(objectiveId, kr.id, currentValue, rating, report, 3, selectedChallengeTags);
        onClose();
    };
        ...
        if (textNode.nodeType === Node.TEXT_NODE && textNode.textContent) {
            ...
            if (slashIndex !== -1) {
                ...
                setTaskMenu({
                    field,
                    filter: query,
                    position: { top: rect.bottom, left: Math.max(0, left) },
    ...
    return (
        <div
            className="fixed top-[61px] left-0 h-[calc(100%-61px)] w-full max-w-lg bg-white shadow-2xl flex flex-col z-40 animate-slide-in-left"
            onClick={e => e.stopPropagation()}
            dir="rtl"
        >
            <div className="p-4 border-b flex justify-between items-center bg-gray-50/70 flex-shrink-0">
                <h2 className="text-lg font-bold text-brand-text truncate">{kr.title}</h2>
                <button onClick={onClose} className="p-1 rounded-full text-gray-400 hover:bg-gray-100">
                    <CloseIcon className="w-5 h-5" />
                </button>
            </div>

            <form onSubmit={handleCheckin} className="flex-grow overflow-y-auto space-y-8 p-6">
                <div className="text-center">
                    <div className="inline-block bg-teal-100/70 p-4 rounded-full mb-2">
                        <TrophyIcon className="w-24 h-24 text-teal-500" />
                    </div>
                </div>
                ...
                <div className="border-t pt-6">
                    <h3 className="text-lg font-semibold text-brand-text mb-4">نمودار تلاش</h3>
                    <ProgressChart objective={{...kr, keyResults: [kr] } as Objective} />
                </div>
                ...
                <div className="border-t pt-6 space-y-4">
                    <div>
                        ...
                        <div ref={tasksDoneRef} id="tasks-done" contentEditable onInput={(e) => handleEditorInput(e, 'done')} className="input-style min-h-[80px] whitespace-pre-wrap" />
                    </div>
                    <div>
                        ...
                        <div ref={tasksNextRef} id="tasks-next" contentEditable onInput={(e) => handleEditorInput(e, 'next')} className="input-style min-h-[80px] whitespace-pre-wrap" />
                    </div>
                    <div>
                        <label htmlFor="challenges" className="block text-sm font-medium text-gray-700 mb-1">چالش‌ها و موانع</label>
                        <textarea ref={challengesRef} id="challenges" rows={4} className="input-style" />
                    </div>
                </div>
                ...
                <div className="border-t pt-6">
                    <h3 className="text-lg font-semibold text-brand-text mb-4">نظرات</h3>
                    <Comments comments={kr.comments || []} users={users} currentUser={currentUser} onAdd={(text) => onAddComment(objectiveId, kr.id, text)} />
                </div>
